      ...
      if (err) {
        ...
        if (err.message.includes("Unexpected end of form")) {
          return res.status(400).json({
            ...
            solution:
              "Check if all fields are properly filled and file is valid",
          });
        }
...
router.put("/apps/:id", upload.single("logo"), async (req, res) => {
  try {
    ...
    if (req.file) {
      updateData.logo = `${req.protocol}://${req.get("host")}/uploads/${
        req.file.filename
      }`;
    }
